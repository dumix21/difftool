...
public class MenuBarFactory {
	...
	public MenuBar getMenuBar(final BorderPane layout, final Stage window) {
		...
		/**
		 * Each time we select file option from menu bar, a pane with two buttons for
		 * selecting paths for files and two text areas will be returned and set in the
		 * main layout
		 */
		...
		/**
		 * Each time we select directory option from menu bar, a pane with two buttons
		 * for selecting paths for directories and two tree viewers will be returned and
		 * set in the main layout
		 */
		...
		/**
		 * This action is handled in two ways: 1. directories comparison 2. files
		 * comparison
		 */
		compare.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				/**
				 * First branch is used for directories if option for directories is set as true
				 * and both directories are chosen those tree views will be compared
				 */
				if (optionDirectory && ((directoryView.getFirstDir().isDirectory()
						&& (directoryView.getLastDir().isDirectory())))) {
					TreeView<Object> tv1 = directoryView.getLeftHelper().getTreeView();
					TreeView<Object> tv2 = directoryView.getRightHelper().getTreeView();
					DirectoriesComaprison compare = new DirectoriesComaprison(directoryView.getFirstDir(),
							directoryView.getLastDir(), tv1, tv2);
					try {
						Platform.runLater(new Runnable() {
							...
							public void run() {
								 ...
								 * The key is given by file name and the value by difference Value can be: 1.
								 * identical, if the checkSum( you can see this function in DirectorieComparison
								 * class) for both files is equal and have the same name
								 * 
								 ...
								 * 
								 */
								HashMap<String, DIFFTYPE> newMap = compare.getMapDiff();
								...
								/**
								 * After the trees were filtered by differences, the new ones will replace the
								 * old ones
								 */
								dd.markDifferences(auxView, directoryView.leftHelper.getTreeView(), newMap,
								...
								dd.addEventHandler(WorkerStateEvent.WORKER_STATE_SUCCEEDED, //
										new EventHandler<WorkerStateEvent>() {
											...
											public void handle(WorkerStateEvent t) {
												directoryView.getStatusLabel().textProperty().unbind();
												directoryView.getStatusLabel()
														.setText("Loaded: " + DirectoriesComaprison.count);
											}
										});
